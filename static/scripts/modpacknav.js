...
function init() {
    ...
    fetch("api/v1/users/", {
        ...
        res.json().then((data) => {
            ...
            if (data.user_id !== undefined) {
                ...
                document.getElementById("ProfilePFP").src = `https://markpacks.net/api/v1/users/${userId}/profile-pictures`;
                ...
                document.getElementById("commentPFPIMG").src = `https://markpacks.net/api/v1/users/${userId}/profile-pictures`;
                menuBackground.style.width = `calc(4.5em + ${(profileTitleContainer.clientWidth / document.documentElement.clientWidth) * 100}vw)`;
                document.getElementById("titleTabs").style.right = `calc(1.75em + ${profileTitleContainer.clientWidth + 1}px)`;
                document.dispatchEvent(loadedEvent);
            } else {
                localStorage.removeItem("user_id");
                document.dispatchEvent(loadedEvent);
    ...
    menuBackground.style.width = `calc(4.5em + ${(profileTitleContainer.clientWidth / document.documentElement.clientWidth) * 100}vw)`;
    document.getElementById("titleTabs").style.right = `calc(1.75em + ${profileTitleContainer.clientWidth + 1}px)`;
    document.querySelector(".loadingScreen").classList.add("hideLoader");
}
...
function fetchModpackData() {
    fetch("https://api.modrinth.com/v2/project/dying-light")
        .then((response) => response.json())
        .then((data) => {
            dataArr[0].downloadText = data.downloads.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
            dataArr[0].downloadText = data.downloads.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
    ...
    fetch("https://api.cfwidget.com/508176")
        .then((response) => response.json())
        .then((data) => {
            dataArr[1].downloadText = data.downloads.total.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
            dataArr[1].downloadText = data.downloads.total.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
        });
    fetch("https://api.modrinth.com/v2/project/rinthed")
        .then((response) => response.json())
        .then((data) => {
            dataArr[2].downloadText = data.downloads.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
            dataArr[2].downloadText = data.downloads.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
    ...
    fetch("https://api.modrinth.com/v2/project/wild")
        .then((response) => response.json())
        .then((data) => {
            dataArr[3].downloadText = data.downloads.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
            dataArr[3].downloadText = data.downloads.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
    ...
    fetch("https://api.cfwidget.com/411167")
        .then((response) => response.json())
        .then((data) => {
            dataArr[4].downloadText = data.downloads.total.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
            dataArr[4].downloadText = data.downloads.total.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ",");
        });
}
...
function changeDetails(data, firstLoad = false) {
    const { selectedOptionIndex, title, summary, downloadText, downloadPage, versionText, prefix = "DL" } = data;
    ...
    if (!firstLoad) {
        ...
        setTimeout(() => {
            ...
            Array.from(document.querySelectorAll(".imageSelected")).forEach((img) => img.classList.remove("imageSelected"));
...
imageContainer.addEventListener("mousemove", function (e) {
    ...
    otherArrows = document.querySelectorAll(".arrow:not(#" + circle.id.substring(0, circle.id.length - 6) + ")");
...
document.addEventListener("mousemove", function (e) {
    ...
    if (document.querySelector(".preTransition3C") !== null) {
        return;
    }

    let xPercent = (e.clientX / window.innerWidth) * 2 - 1;
    let yPercent = (e.clientY / window.innerHeight) * 8 - 4;

    let translateX = Math.max(-20, Math.min(20, xPercent));
    let translateY = Math.max(-15, Math.min(15, yPercent));

    imageGallery.style.transform = "translateX(" + translateX * 5 + "%) translateY(" + translateY + "%)";
});
